...
for(chr in 1:22){
  ...
  for(file in list_ped_by_chr){
    ...
    agg.genos.by.fam = tryCatch(agg.genos.by.fam(pedfile.path=NULL, pedfile=pedfile, correction="replace"),
                                error = function(e) e)
    ...
    results = RetroFun.RVS(null.without.consanguinity, agg.genos.by.fam, Z_annot = annotation.matrix, W = rep(1, nrow(annotation.matrix)), independence=FALSE)
    print(results)
    n_CRH <- 1
...
saveRDS(genome_results, paste0(output,"/RetroFun.RVS_results_CRHs_seq_all_chromosomes_without_consanguinity_replace.RDS"))
