...
# dist ne sert à rien dans cette version
...
region <- data.table::fread("/lustre09/project/6033529/schizo/data/WGS_bs_2022/500_samples_cag/regionsFamilles.txt", header = TRUE)
...
#Split problematic families for GCbr. We now have a list of pedigrees.
# Modifier pour aller chercher les bons indices de familles pour tout phénotype
#fam_prob <- fam_idx %in% c(10, 18, 47)

if (pheno %in% c("GCbr","GCna")) fam_prob <- pedigree$famid[1] %in% c("119", "131", "255") else fam_prob = FALSE
if (fam_prob){
  #source("/lustre03/project/6033529/quebec_10x/scripts/WGS_bs_2022_500samples/call/RetroFunRVS_null_object_split_fam_prob.R")
  source("/lustre09/project/6033529/schizo/scripts_AB//RetroFunRVS_null_object_split_fam_prob.R")
  print(pedigree); print("*** Pedigree after splitting ***"); print(split_pedigree)
  pedigree <- split_pedigree
  saveRDS(pedigree, paste0(pathAB_ped, "/fam", fam, "splitted.rds"))
}
...
if(cryptic){
  ...
  saveRDS(expected.variance.seq.fam, file = paste0(pathAB_ped, paste0("/expected.variance.consanguinity.cryptique.",pheno,".fam", fam_idx, out_dist, ".rds")))
  ...
  attributes(expected.variance.seq.fam)$distinguishHomo = T
  saveRDS(expected.variance.seq.fam, file = paste0(pathAB_ped, paste0("/expected.variance.consanguinity.",pheno,".fam", fam_idx, out_dist, ".rds")))
}else{
  expected.variance.seq.fam <- suppressMessages(compute.null.parallel(pedigree = pedigree, distinguishHomo = consanguinity, cryptic.relatedness = cryptic, out.prob.dist = dist))
  saveRDS(expected.variance.seq.fam, file = paste0(pathAB_ped, paste0("/expected.variance.",pheno,".fam", fam_idx, out_dist, ".rds")))
}