...
process.ABC = function(ABC.inputFile, genebody=FALSE) {
  ...
  if(genebody==TRUE) {
    df.gene = data.frame(genesInRRCs)
    df.gene = df.gene[,c("seqnames", "start", "end", "geneSymbol")]
    colnames(df.gene) = c("chr", "startGene", "endGene", "TargetGene")
    ...
    enhancers.promoters = merge(merge(ABC.input.WX, df.promoters, by="TargetGene", all.x=TRUE), df.gene, by="TargetGene", all.x=TRUE)
  }
