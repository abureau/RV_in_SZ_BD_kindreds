#This code allows the production of post analyses in the real-data application on SZ BP pedigrees 
#Paths and directories have to be changed, all data for lifting over are present in the repo

#Functions from CRHs.R have to be imported 
source("CRHs.R")
...
ABC_Pred = process.ABC("C:\\Users\\loicm\\Documents\\CRHs_SZ\\data\\EnhancerPredictions_NEU_iPSC.txt",genebody = T)
...
#We remove regions located on other chromosomes
...
liftover_hg38_GenesBody = read.table("C:\\Users\\loicm\\Downloads\\hglft_genome_12321_7a2b30.bed", header=FALSE)
colnames(liftover_hg38_GenesBody) = c("chrom_on_hg38","start_hg38","end_hg38","TargetGene","n_correspondance", "strand")

GRanges_GenesBody_liftover_hg38 = GRanges(seqnames = liftover_hg38_GenesBody$chrom_on_hg38 ,ranges=IRanges(start=liftover_hg38_GenesBody$start_hg38,end=liftover_hg38_GenesBody$end_hg38), TargetGene=liftover_hg38_GenesBody$TargetGene)
...
df_GenesBody.hg38.final = as.data.frame(GRanges_GenesBody_liftover_hg38)
colnames(df_GenesBody.hg38.final) = c("chr.x","startGene.hg38","endGene.hg38","widthGene.hg38","strand","TargetGene")

#Set commons levels for chr.x avoiding weird behaviors of the merge function
ABC_Pred.for.liftover$chr.x = factor(ABC_Pred.for.liftover$chr.x, levels=paste0("chr",1:22))
df_GenesBody.hg38.final$chr.x = factor(df_GenesBody.hg38.final$chr.x, levels = paste0("chr",1:22))
df_Enhancers.hg38.final$chr.x = factor(df_Enhancers.hg38.final$chr.x, levels=paste0("chr",1:22))
df_Genes.hg38.final$chr.x = factor(df_Genes.hg38.final$chr.x, levels=paste0("chr",1:22))

ABC_Pred.for.liftover = merge(merge(merge(ABC_Pred.for.liftover, df_Enhancers.hg38.final[,c("chr.x","start.hg38","end.hg38", "name")], by=c("chr.x", "name"), all.x=TRUE), df_Genes.hg38.final[,c("chr.x","startProm.hg38","endProm.hg38","TargetGene")], by=c("chr.x","TargetGene"), all.x=TRUE), na.omit(df_GenesBody.hg38.final[!duplicated(df_GenesBody.hg38.final[, c("chr.x","TargetGene")]), c("chr.x", "startGene.hg38", "endGene.hg38", "TargetGene")]), by=c("chr.x","TargetGene"), all.x=T)
ABC_Pred.for.liftover.tmp = ABC_Pred.for.liftover
ABC_Pred.for.liftover = ABC_Pred.for.liftover[,c("chr.x","start.hg38","end.hg38", "name","startProm.hg38","endProm.hg38","startGene.hg38","endGene.hg38","TargetGene","TargetGeneTSS", "CellType","ABC.Score",     "typeOf", "chr.y")]
colnames(ABC_Pred.for.liftover)=c("chr.x","start", "end", "name", "startProm","endProm","startGeneBody","endGeneBody", "TargetGene","TargetGeneTSS", "CellType","ABC.Score",     "typeOf", "chr.y")
...
write.table(ABC_Pred_with_membership.liftover.hg38, "C:\\Users\\loicm\\Documents\\CRHs_SZ\\EnhancerPredictions_NEU_iPSC_liftoverhg38_withGeneBody.txt", col.names = TRUE, row.names = TRUE, sep="\t", quote=F)
...
GGally::ggnet2(decompose_CRHs_iPSC_NEU.liftover.hg38[[1438]], size=4,color="Elements", shape="Elements", palette = "Set1",label="Name",label.size = 3,legend.size = 20,
